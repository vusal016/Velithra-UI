...
export default function EditRolePage() {
  ...
  const loadRole = async () => {
    try {
      setLoading(true);
      const role = await roleManagementService.getById(roleId);
      setFormData({
        id: role.id,
        name: role.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await roleManagementService.update(formData.id, formData);
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-300 mt-1">Update role information</p>
          </div>
        </div>
